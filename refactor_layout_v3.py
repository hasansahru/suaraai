...
# We need to manually close the divs in input_part1
# input_part1 has `<div className="azure-card relative">` and `<div className="space-y-4">`
...
# REMOVE the last two </div> from submit_block
...
# REMOVE the </section> from results_part
...
new_bento_grid = """
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
          ...
          <div className="azure-card p-6 rounded-2xl h-full flex flex-col justify-start">
            ...
            """ + notes_block.replace('<div className="azure-card p-5 border border-border/60">', '').replace('</div>', '', 1) + """
          </div>
        </div>
"""
