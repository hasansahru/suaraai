...
async def api_tts(req: TTSRequest):
    ...
    try:
        ai_provider_setting = load_json_setting("ai_provider_setting.json")
        provider_options = ai_provider_setting.get("providers", [])
        active_prov = next((p for p in provider_options if p["id"] == req.provider_id), {})
    except Exception:
        pass

    resolved_base_url = req.base_url or active_prov.get("default_base_url", "")
    if not resolved_base_url:
        raise HTTPException(status_code=400, detail="Base URL tidak ditemukan.")
    
    if resolved_base_url.endswith("/"):
        resolved_base_url = resolved_base_url[:-1]
        
    url = f"{resolved_base_url}/audio/speech"
    headers = {"Content-Type": "application/json"}
    
    api_key = req.api_key
    if not api_key:
        api_key_env = active_prov.get("api_key_env", "")
        if api_key_env:
    ...
    payload = {
        "model": req.model,
        "input": req.text[:4000],  # safety cap
        "voice": req.voice
    }
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            return {"status": "success", "audio_base64": audio_b64, "mime_type": "audio/mpeg"}
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Gagal generate TTS: {str(e)}")
