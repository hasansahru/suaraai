...
8. Semua estimasi waktu (`start_estimate` dan `end_estimate` pada outline `video_panjang`) WAJIB berupa durasi nyata dalam format `mm:ss` atau `hh:mm:ss` yang dihitung secara kumulatif, mulai dari 00:00 pada babak pertama dan berakhir tepat di dalam rentang Durasi Target video baru yang dipilih pengguna. JANGAN menggunakan nilai placeholder deskripsi (seperti "hh:mm:ss") atau membiarkan seluruh babak bernilai 00:00.
