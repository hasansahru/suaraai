...
- **Presisi Timestamp & Sinkronisasi Teks-Audio Sumber**: Seluruh timestamp (`sumber_start`, `sumber_end`, `sumber_segmen`, dan `segmen.start_time`/`end_time`) WAJIB bersumber 100% presisi dari timestamp audio/transkrip asli. **Jika baris transkrip disertai prefix timestamp asli `[mm:ss - mm:ss]`, SALIN angka tersebut langsung dari prefix baris pertama dan baris terakhir tempat kutipan verbatim Anda berada — DILARANG KERAS memperkirakan/menghitung dari jumlah kata atau tempo bicara.** Jika transkrip TIDAK ber-timestamp (teks polos), baru estimasi dari posisi kata × tempo bicara 2-3 kata/detik. DILARANG KERAS mengarang timestamp fiktif atau memasangkan kutipan narasi yang durasi bacanya tidak seimbang dengan rentang timestamp sumber!
- **Kutipan Asli Transkrip (`narasi_sumber` / `kutipan`)**: Setiap narasi yang dikutip dari video sumber wajib berupa teks VERBATIM (persis kata per kata, tanpa menambah/mengurangi kata atau mengubah urutan kata; boleh dipotong di batas kalimat) yang benar-benar ada di transkrip pada rentang `sumber_start` hingga `sumber_end` tersebut, diambil dari AWAL rentang, dengan jumlah kata yang realistis terhadap durasi audio. Backend memakai kutipan ini untuk memvalidasi dan mengoreksi otomatis timestamp sumber (fuzzy matching) — kutipan yang tidak cocok dengan transkrip membuat timestamp Anda tidak tervalidasi.
