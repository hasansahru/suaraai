...
7. **Start Time & End Time — ATURAN KETAT, WAJIB DIPATUHI (khusus segmen/shot Shorts)**: `end_time - start_time` **HARUS** sedekat mungkin dengan "Durasi Target" yang diminta pengguna (toleransi ±10 detik saja). **DILARANG KERAS** membuat segmen yang durasinya jauh lebih pendek dari Durasi Target (misal: jika pengguna memilih durasi target 60 detik, durasi segmen TIDAK BOLEH di bawah 50 detik). Jika durasi di transkrip kurang dari target, lebarkan rentang waktu pengambilan klip ke kalimat sebelum atau sesudahnya agar durasi totalnya pas dengan target. Contoh SALAH: target 60 detik tapi menghasilkan segmen berdurasi 15-20 detik. Contoh BENAR: target 60 detik → `start_time=05:42, end_time=06:39` (durasi 57 detik), atau `start_time=01:10, end_time=02:08` (durasi 58 detik). Sebaliknya, jika blok topik terlalu panjang, potong menjadi sub-bagian yang pas dengan target durasi.
